...
const FEJLTEKSTER: Record<number, string> = {
  ...
  422: "Kontrakten mangler håndværkerens e-mail.",
};
...
export default function HaandvaerkerKontraktChat({
  params,
}: {
  params: Promise<{ id: string; kontraktId: string }>;
}) {
  ...
  const hentBeskeder = useCallback(
    async (accessToken: string) => {
      const res = await fetch(
        `/api/chat/beskeder?kontrakt_id=${encodeURIComponent(kontraktId)}`,
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-white border-b border-gray-100 px-6 py-3.5 sticky top-0 z-50 flex-shrink-0">
      ...
      <div className="flex flex-col flex-1 overflow-hidden" style={{ height: "calc(100vh - 57px)" }}>
        {/* Samtale-header */}
        <div className="bg-white border-b border-gray-100 px-5 py-3.5 flex items-center flex-shrink-0">
          <div className="min-w-0">
            <p className="font-semibold text-gray-900 text-sm leading-none">Samtale med bygherre</p>
                  ...
                  <div key={b.id} className={`flex gap-3 mb-3 ${erMig ? "flex-row-reverse" : "flex-row"}`}>
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-1 ${
                        erMig ? "bg-[#1e3a2a] text-white" : "bg-gray-200 text-gray-700"
                      }`}
                    >
